...
export default function WelcomeHeroSection({ userName, onStartLearning, isDark = false }) {
  ...
  return (
    <motion.section
      ...
      className={`relative rounded-3xl border overflow-hidden min-w-0 ${
        isDark 
          ? 'liquid-glass-dark' 
    ...
    >
      <motion.div
        ...
        className={`absolute -left-20 -bottom-24 h-56 w-56 rounded-full blur-3xl ${
          isDark ? 'bg-cyan-500/20' : 'bg-cyan-200/60'
        }`}
      />
      ...
      <div className={`relative z-10 grid md:grid-cols-[1fr_auto] gap-5 items-center p-5 md:p-8 min-w-0`}>
        <div className="min-w-0">
          ...
          <h1 className={`text-2xl md:text-3xl font-semibold leading-tight mb-2 break-words ${
            isDark ? 'text-slate-100' : 'text-slate-900'
          }`}>
            {getGreeting()}, {userName}
          </h1>

          <p className={`max-w-2xl text-sm md:text-base break-words ${
