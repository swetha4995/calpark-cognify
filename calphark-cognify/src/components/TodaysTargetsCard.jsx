    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Stay consistent with daily milestones.</p>
        </div>
      ...
      <div className="space-y-2.5 mb-4">
        {targets.map((target) => (
          <div key={target.id} className={`flex items-center gap-2.5 rounded-lg px-3 py-2 ${
            ...
            <span className={`text-sm ${target.done ? (isDark ? 'text-slate-200' : 'text-slate-700') : (isDark ? 'text-slate-300' : 'text-slate-600')}`}>
              {target.label}
            </span>
          </div>
        ))}
