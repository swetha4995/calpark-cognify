...
export default function AIMentorButton({ isDark = false }) {
  ...
  const handleSendMessage = (e) => {
    ...
    const context = [
      {
        role: 'system',
        content:
          'You are an educational AI mentor for school students. Keep answers clear, concise, and step-by-step. Encourage learning and confidence.',
      },
    ...
    getGroqChatReply(context).then((reply) => {
      setMessages((prev) => [
        ...prev,
            ...
            <div className={`flex-1 p-4 overflow-y-auto space-y-3 scroll-smooth ${isDark ? 'bg-slate-900/50' : 'bg-slate-50/30'}`}>
              {messages.map((msg, idx) => (
                ...
                >
                  ...
                  >
                    <p className={msg.type === 'user' ? 'text-white' : isDark ? 'text-slate-200' : 'text-slate-700'}>
                      {msg.text}
                    </p>
                  </div>
                </motion.div>
            ...
            >
              <div className="flex gap-2">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white inline-flex items-center justify-center transition"
                >
